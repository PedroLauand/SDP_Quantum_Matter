"""Readable example script for the real-restricted level-3 W robustness SDP."""
...
try:
    from level3_real_restricted_model import (
        DEFAULT_SCS_ACCELERATION_INTERVAL,
        DEFAULT_SCS_ACCELERATION_LOOKBACK,
        DEFAULT_SCS_ALPHA,
        DEFAULT_SCS_RHO_X,
        DEFAULT_SCS_SCALE,
        build_inflation_robustness_level3_real_restricted_scs_model,
        solve_inflation_robustness_level3_real_restricted_scs,
    )
except ModuleNotFoundError:
    from Baroque_test_paper.level3_real_restricted_model import (
        DEFAULT_SCS_ACCELERATION_INTERVAL,
        DEFAULT_SCS_ACCELERATION_LOOKBACK,
        DEFAULT_SCS_ALPHA,
        DEFAULT_SCS_RHO_X,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Build or solve the real-restricted SCS level-3 inflation robustness SDP for W.",
    )
    ...
    solved = solve_inflation_robustness_level3_real_restricted_scs(
        rho,
        dims3,
        verbose=args.verbose,
        optimizer_max_time=args.optimizer_max_time,
    ...
    print(f"tMax = {t_value:.8f}", flush=True)
    if t_value < 1.0:
