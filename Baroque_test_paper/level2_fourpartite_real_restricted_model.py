"""Readable four-partite level-2 real-restricted robustness SDP.

This is the real-symmetric restriction of ``level2_fourpartite_complex_model.py``.
...
Both are declared directly in the real-symmetric coordinate basis.
"""
...
@dataclass
class InflationRobustnessLevel2FourpartiteRealRestrictedSCSModel:
...
# Slot order for sigma: (A111, B111, C111, D111, A222, B222, C222, D222)
# Slot order for tau:   (A211, B111, C111, D112, A122, B222, C222, D221)
SIGMA_SWAP = (4, 5, 6, 7, 0, 1, 2, 3)
TAU_HALFTURN = (4, 5, 6, 7, 0, 1, 2, 3)
OBSERVED_ANCHOR_SIGMA_KEEP = (0, 1, 2, 3)
EQ_SIGMA_TAU_ABC_SIGMA = (0, 1, 2, 4, 5, 6)
EQ_SIGMA_TAU_ABC_TAU = (0, 1, 2, 4, 5, 6)
EQ_SIGMA_TAU_ABD_SIGMA = (0, 1, 3, 4, 5, 7)
EQ_SIGMA_TAU_ABD_TAU = (0, 1, 3, 4, 5, 7)
EQ_SIGMA_TAU_ACD_SIGMA = (0, 2, 3, 4, 6, 7)
EQ_SIGMA_TAU_ACD_TAU = (0, 2, 3, 4, 6, 7)
EQ_SIGMA_TAU_BCD_SIGMA = (1, 2, 3, 5, 6, 7)
EQ_SIGMA_TAU_BCD_TAU = (1, 2, 3, 5, 6, 7)
...
def _validate_level2_fourpartite_inputs(
    ...
    if not np.allclose(np.imag(rho), 0.0, atol=1e-9):
        raise ValueError("Real-restricted formulation requires a real target state.")
    dims8 = (
...
def build_inflation_robustness_level2_fourpartite_real_restricted_scs_model(
    rho: np.ndarray,
    dims4: tuple[int, int, int, int],
    *,
    verbose: int = 1,
    optimizer_max_time: float | None = None,
) -> InflationRobustnessLevel2FourpartiteRealRestrictedSCSModel:
    """Build the direct SCS cone program for the real-restricted four-partite level-2 SDP."""
    ...
    builder.add_difference_equality(
        sigma,
        cached_real_symmetric_coordinate_action_for_slot_permutation(dims8, SIGMA_SWAP),
        sigma,
    ...
    builder.add_difference_equality(
        tau,
        cached_real_symmetric_coordinate_action_for_slot_permutation(dims8, TAU_HALFTURN),
        tau,
    ...
    builder.add_observed_anchor(
        sigma,
        cached_real_symmetric_partial_trace_coordinate_action(dims8, OBSERVED_ANCHOR_SIGMA_KEEP),
    ...
    builder.add_difference_equality(
        sigma,
        cached_real_symmetric_partial_trace_coordinate_action(dims8, EQ_SIGMA_TAU_ABC_SIGMA),
        tau,
        cached_real_symmetric_partial_trace_coordinate_action(dims8, EQ_SIGMA_TAU_ABC_TAU),
        counter_key="equality",
    )
    builder.add_difference_equality(
        sigma,
        cached_real_symmetric_partial_trace_coordinate_action(dims8, EQ_SIGMA_TAU_ABD_SIGMA),
        tau,
        cached_real_symmetric_partial_trace_coordinate_action(dims8, EQ_SIGMA_TAU_ABD_TAU),
        counter_key="equality",
    )
    builder.add_difference_equality(
        sigma,
        cached_real_symmetric_partial_trace_coordinate_action(dims8, EQ_SIGMA_TAU_ACD_SIGMA),
        tau,
        cached_real_symmetric_partial_trace_coordinate_action(dims8, EQ_SIGMA_TAU_ACD_TAU),
        counter_key="equality",
    )
    builder.add_difference_equality(
        sigma,
        cached_real_symmetric_partial_trace_coordinate_action(dims8, EQ_SIGMA_TAU_BCD_SIGMA),
        tau,
        cached_real_symmetric_partial_trace_coordinate_action(dims8, EQ_SIGMA_TAU_BCD_TAU),
        counter_key="equality",
    )
    ...
    builder.add_auxiliary_link(
        ppt_sigma_full,
        sigma,
        cached_real_symmetric_partial_transpose_coordinate_action(dims8, PPT_SIGMA_FULL_TRANSPOSE),
    )

    # 6) Linear bounds and PSD cone constraints.
    zero_rows = int(builder.next_row)
    builder.add_scalar_nonnegative(p_slice)
    _add_scalar_upper_bound(builder, p_slice, 1.0)
    ...
    cone = {"z": int(zero_rows), "l": 2, "s": s_dims}
    ...
    build_profile = {
        ...
        "scope": {
            "formulation": "level2_fourpartite_real_restricted_scs",
            "full_dim": full_dim,
    ...
    return InflationRobustnessLevel2FourpartiteRealRestrictedSCSModel(
        dims4=dims4,
        dims8=dims8,
        D=D,
        full_dim=full_dim,
...
def solve_inflation_robustness_level2_fourpartite_real_restricted_scs(
    rho: np.ndarray,
    dims4: tuple[int, int, int, int],
    *,
    verbose: int = 1,
    ...
    built = build_inflation_robustness_level2_fourpartite_real_restricted_scs_model(
        rho,
        dims4,
        verbose=verbose,
...
def inflation_robustness_level2_fourpartite_real_restricted(
    rho: np.ndarray,
    dims4: tuple[int, int, int, int],
    *,
    verbose: int = 1,
    ...
    solved = solve_inflation_robustness_level2_fourpartite_real_restricted_scs(
        rho,
        dims4,
        verbose=verbose,
        optimizer_max_time=optimizer_max_time,
