"""Readable level-2 real-restricted robustness SDP.

This is the real-symmetric restriction of ``level2_complex_model.py``.
...
@dataclass
class InflationRobustnessLevel2RealRestrictedSCSModel:
...
# Slot order for sigma: (A11, A22, B11, B22, C11, C22)
# Slot order for tau:   (A11, A22, B11, B22, C12, C21)
SIGMA_SWAP = (1, 0, 3, 2, 5, 4)
TAU_SWAP = (1, 0, 3, 2, 5, 4)
OBSERVED_ANCHOR_SIGMA_KEEP = (0, 2, 4)
EQ_SIGMA_TAU_ABAB_SIGMA = (0, 2, 1, 3)
EQ_SIGMA_TAU_ABAB_TAU = (0, 2, 1, 3)
EQ_SIGMA_TAU_BCBC_SIGMA = (2, 4, 3, 5)
EQ_SIGMA_TAU_BCBC_TAU = (2, 4, 3, 5)
EQ_SIGMA_TAU_CACA_SIGMA = (4, 0, 5, 1)
EQ_SIGMA_TAU_CACA_TAU = (4, 1, 5, 0)
...
def build_inflation_robustness_level2_real_restricted_scs_model(
    rho: np.ndarray,
    dims3: tuple[int, int, int],
    *,
    verbose: int = 1,
    optimizer_max_time: float | None = None,
) -> InflationRobustnessLevel2RealRestrictedSCSModel:
    """Build the direct SCS cone program for the real-restricted level-2 SDP."""
    ...
    builder.add_difference_equality(
        sigma,
        cached_real_symmetric_coordinate_action_for_slot_permutation(dims6, SIGMA_SWAP),
        sigma,
    ...
    builder.add_difference_equality(
        tau,
        cached_real_symmetric_coordinate_action_for_slot_permutation(dims6, TAU_SWAP),
        tau,
    ...
    builder.add_observed_anchor(
        sigma,
        cached_real_symmetric_partial_trace_coordinate_action(dims6, OBSERVED_ANCHOR_SIGMA_KEEP),
    ...
    builder.add_difference_equality(
        sigma,
        cached_real_symmetric_partial_trace_coordinate_action(dims6, EQ_SIGMA_TAU_ABAB_SIGMA),
        tau,
        cached_real_symmetric_partial_trace_coordinate_action(dims6, EQ_SIGMA_TAU_ABAB_TAU),
        counter_key="equality",
    )
    builder.add_difference_equality(
        sigma,
        cached_real_symmetric_partial_trace_coordinate_action(dims6, EQ_SIGMA_TAU_BCBC_SIGMA),
        tau,
        cached_real_symmetric_partial_trace_coordinate_action(dims6, EQ_SIGMA_TAU_BCBC_TAU),
        counter_key="equality",
    )
    builder.add_difference_equality(
        sigma,
        cached_real_symmetric_partial_trace_coordinate_action(dims6, EQ_SIGMA_TAU_CACA_SIGMA),
        tau,
        cached_real_symmetric_partial_trace_coordinate_action(dims6, EQ_SIGMA_TAU_CACA_TAU),
        counter_key="equality",
    )
    ...
    builder.add_auxiliary_link(
        ppt_sigma_full,
        sigma,
        cached_real_symmetric_partial_transpose_coordinate_action(dims6, PPT_SIGMA_FULL_TRANSPOSE),
    )
    ...
    build_profile = {
        ...
        "scope": {
            "formulation": "level2_real_restricted_scs",
            "full_dim": full_dim,
    ...
    return InflationRobustnessLevel2RealRestrictedSCSModel(
        dims3=dims3,
        dims6=dims6,
        D=D,
        full_dim=full_dim,
...
def solve_inflation_robustness_level2_real_restricted_scs(
    rho: np.ndarray,
    dims3: tuple[int, int, int],
    *,
    verbose: int = 1,
    ...
    built = build_inflation_robustness_level2_real_restricted_scs_model(
        rho,
        dims3,
        verbose=verbose,
...
def inflation_robustness_level2_real_restricted(
    rho: np.ndarray,
    dims3: tuple[int, int, int],
    *,
    verbose: int = 1,
    ...
    solved = solve_inflation_robustness_level2_real_restricted_scs(
        rho,
        dims3,
        verbose=verbose,
        optimizer_max_time=optimizer_max_time,
