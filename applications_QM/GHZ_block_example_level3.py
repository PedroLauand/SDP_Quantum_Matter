"""Level-3 noisy-GHZ optimization with the default real block formulation.
...
if __name__ == "__main__":
    print("[GHZ level3 real] building GNME problem", flush=True)
    problem = GNMEProblem(
        n_parties=3,
        inflation_level=3,
    ...
    print("[GHZ level3 real] building SDP draft", flush=True)
    draft = build_sdp_draft(problem)

    print("[GHZ level3 real] solving optimization over p", flush=True)
    result = solve_min_noise_level(draft, Hermitian=HERMITIAN, verbose=PROGRESS_VERBOSE)

    print("GNME noisy-GHZ optimization (level 3)")
