...
def solve_min_noise_level(draft, Hermitian: bool, verbose: int):
    ...
    for target_lexorder, w_matrix in w_targets.items():
        ...
        w_coord = matrix_coordinate_values(w_matrix, Hermitian)
        mixed_coord = matrix_coordinate_values(mixed_targets[target_lexorder], Hermitian)
        delta_coord = mixed_coord - w_coord
        for coord_index, (base_value, delta_value) in enumerate(zip(w_coord, delta_coord)):
...
def print_result(level: int, Hermitian: bool, result: dict) -> None:
    print(f"GNME noisy-W optimization (level {level})")
