"""Level-2 noisy-GHZ optimization with the Hermitian block formulation.

This example uses the same default GNME package API but sets `Hermitian=True`.
...
if __name__ == "__main__":
    print("[GHZ level2 Hermitian] building GNME problem", flush=True)
    problem = GNMEProblem(
        n_parties=3,
        inflation_level=2,
    ...
    print("[GHZ level2 Hermitian] building SDP draft", flush=True)
    draft = build_sdp_draft(problem)

    print("[GHZ level2 Hermitian] solving optimization over p", flush=True)
